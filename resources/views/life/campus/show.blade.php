@extends('life.campus.campus_base')
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        <div class="col-sm-8">
          <h3 class="box-title">The Campus</h3>
        </div>
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('life_jim.create_campus') }}">Add New</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
    ...
    <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
      <div class="row">
        <div class="col-sm-12">
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            <tbody>
            @foreach ($campus as $key=>$s)
                <tr role="row" class="odd">
                <td class="sorting_1">{{ $key+1 }}</td>
                <td class="sorting_1">{{ $s->title }}</td>
                  <td>
                    <form class="row" method="POST" action="{{ route('life_jim.destroy_campus', ['id' => $s->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <!-- <input type="hidden" name="_method" value="DELETE"> -->
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('life_jim.edit_campus', ['id' => $s->id]) }}" class="btn btn-warning col-sm-3 col-xs-5 btn-margin">
                        Update
                        </a>
      ...
      <div class="row">
        <div class="col-sm-5">
          <div class="dataTables_info" id="example2_info" role="status" aria-live="polite">Showing 1 to {{count($campus)}} of {{count($campus)}} entries</div>
        </div>
        <div class="col-sm-7">
