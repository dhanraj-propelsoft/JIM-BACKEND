...
@section('action-content')
  ...
  <div class="box-body">
    ...
    <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($sliders as $key=>$slider)
                ...
                <td class="sorting_1">{{ $key+1 }}</td>
                  ...
                  <td class="hidden-xs">{{ $slider->content }}</td>
                  <td><img src="{{$slider->image}}" width="50px" height="50px"/></td>
                  <td>
                    <form class="row" method="POST" action="{{ route('home-page.destroy_slider', ['id' => $slider->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <!-- <input type="hidden" name="_method" value="DELETE"> -->
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('home-page.edit_slider', ['id' => $slider->id]) }}" class="btn btn-warning col-sm-3 col-xs-5 btn-margin">
                        Update
                        </a>
      ...
      <div class="row">
        <div class="col-sm-5">
          <div class="dataTables_info" id="example2_info" role="status" aria-live="polite">Showing 1 to {{count($sliders)}} of {{count($sliders)}} entries</div>
        </div>
        <div class="col-sm-7">
